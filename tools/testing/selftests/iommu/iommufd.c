					 ...
					 unsigned int access_item_id)
{
	struct iommu_test_cmd cmd = {
		...
		.id = access_id,
		.destroy_access_pages = { .access_pages_id = access_item_id },
	};
...
#define test_cmd_destroy_access_item(access_id, access_item_id)         \
	ASSERT_EQ(0, _test_cmd_destroy_access_item(self->fd, access_id, \
						   access_item_id))
#define test_err_destroy_access_item(_errno, access_id, access_item_id) \
	EXPECT_ERRNO(_errno, _test_cmd_destroy_access_item(             \
				     self->fd, access_id, access_item_id))
...
TEST_F(iommufd_ioas, access_pin)
{
	...
	for (npages = 1; npages < BUFFER_SIZE / PAGE_SIZE; npages++) {
		...
		ASSERT_EQ(0, ioctl(self->fd,
				   _IOMMU_TEST_CMD(IOMMU_TEST_OP_ACCESS_PAGES),
				   &access_cmd));
		access_item_id = access_cmd.access_pages.out_access_pages_id;
		ASSERT_EQ(0, ioctl(self->fd,
				   _IOMMU_TEST_CMD(IOMMU_TEST_OP_ACCESS_PAGES),
				   &access_cmd));
		test_cmd_destroy_access_item(
